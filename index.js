...
function replaceText(word, replacement, text) {
  const regexp = /word/g; // Створення регулярного виразу для пошуку слова з флагом 'g' (глобальний пошук).
  return text.replace(word, replacement); // Використання методу `replace` регулярного виразу для заміни слова на фразу у тексті.
...
function checkWord(word, text) {
  const regexp = new RegExp(word, "i"); // Створення регулярного виразу для пошуку слова з флагом 'i' (регістронезалежний пошук).
  return text.search(regexp) !== -1; // Використання методу `test` регулярного виразу для перевірки наявності слова у тексті.
...
function extractTextInParentheses(str) {
  const regex = /\((.*?)\)/g; // Регулярний вираз для пошуку тексту в круглих дужках.
  const matches = str.matchAll(regex); // Використання методу `matchAll` для отримання всіх збігів регулярного виразу.

  const extractedTexts = []; // Масив для збереження вилучених текстів.

  for (const match of matches) {
    extractedTexts.push(match[1]); // Додавання знайденого тексту в масив вилучених текстів.
  }
...
function countEmails(str) {
  const regex = /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/g; // Регулярний вираз для пошуку email-адрес.
  const matches = str.match(regex); // Використання методу `match` для отримання всіх збігів регулярного виразу.

  if (matches) {
    return matches.length; // Повернення кількості email-адрес.
  } else {
    return 0; // Якщо не знайдено жодного email-адресу, повернути 0.
  }
...
function findWordOccurrences(str, word) {
  const regex = new RegExp(word, "gi"); // Регулярний вираз для пошуку слова з флагами 'g' (глобальний пошук) та 'i' (ігнорування регістру).
  const matches = [];
  let match;

  while ((match = regex.exec(str)) !== null) {
    matches.push(match.index); // Додавання індексу поточного входження слова у масив.
    regex.lastIndex = match.index + 1; // Оновлення lastIndex для продовження пошуку з наступного символу.
  }

  return matches; // Повертаємо масив з індексами входжень слова.
...
function checkRegexFlags(regex) {
  // Отримуємо всі флаги регулярного виразу за допомогою свойства `flags`.
...
function replaceWordOccurrences(str, word, newWord) {
  // Створюємо регулярний вираз зі словом та флагом 'g' для глобального пошуку всіх входжень.
